...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):
        ...
        """
        """YOUR CODE"""
        inActs.insert(0,1.0) #align the weight for bias input
        weightedSum = self.getWeightedSum(inActs)
        inActs.pop(0)#pop the extra element
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i, weight in enumerate(self.weights):
            updatedWeight = weight + (alpha * delta * inActs[i])
            weights.append(updatedWeight)
            totalModification += abs(updatedWeight - weight)
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        """
        """YOUR CODE"""
        output = [inActs] #initiallize the 1st element of output as inActs
        input = inActs

        for layer in self.layers: #iterate through layers
            curr = []
            for perceptron in layer: # iterate trough all perceptrons in a layer
                curr.append(perceptron.sigmoidActivation(input))
            output.append(curr)
            input = curr
        ...
        for example in examples:#for each example
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    for perceptronNum in range(len(nextLayer)): #iterate through next layer for error and delta
                        delta = nextLayer[perceptronNum].weights[neuronNum+1] * deltas[0][perceptronNum]
                        error += delta
                    delta = gPrime * error
                    """
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    while iteration < maxItr and weightMod > weightChangeThreshold:
        iteration += 1
        trainError, weightMod = nnet.backPropLearning(examplesTrain,alpha) #back propagate with current weights
        #if iteration%10==0:
    ...
    #feed forward iterations
    for input, output in examplesTest:
        netOutput = nnet.feedForward(input)[-1]#iterate backwards
        if [round(i) for i in netOutput] == output:
            testCorrect += 1
        else:
            testError += 1
