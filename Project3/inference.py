#Colaboration with Wantian Zhao and Borun Song on the implementation 
# of initializeUniformly() and observeUpdate() in class {JointParticleFilter(ParticleFilter)}
#Reference to https://docs.python.org/3/library/itertools.html on the use of itertools.product()
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for pos in self.allPositions:
            beliefs[pos] = self.getObservationProb(observation,PacPos,pos,self.getJailPosition()) * self.beliefs[pos]
    ...
    def elapseTime(self, gameState):
        ...
        beliefs = util.Counter()
        for pos in self.allPositions:
            model = self.getPositionDistribution(gameState,pos)
            for newPos in model :
                beliefs[newPos] += model[newPos] * self.beliefs[pos]
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            beliefs[particle] += self.getObservationProb(observation, pacPos, particle, self.getJailPosition())
    ...
    def elapseTime(self, gameState):
        ...
        for x in range(self.numParticles):
            beliefs = self.getPositionDistribution(gameState, self.particles[x])
            self.particles[x] = DiscreteDistribution.sample(beliefs)
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        beliefs = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for pos in self.particles:
            ...
            for n in range(self.numGhosts):
                possibility *= self.getObservationProb(observation[n],pacPos,pos[n],self.getJailPosition(n))
            distribution[pos] += possibility
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for n in range(self.numGhosts):
                newPosDist = self.getPositionDistribution(gameState, oldParticle, n, self.ghostAgents[n])
                newParticle[n] = newPosDist.sample()
